    ...
    <div class="row">
		...
		<form name="frm">
			...
			<div class="col-lg-6 col-sm-6 col-xs-12  detail-right">
				...
				<!--<p class="description">텍스트로 적는 간단 상품 설명 <br>생활용품 17종을 담은 세트 상품입니다.</p>-->

				<?//if ($_SESSION['C_CP_NO'] <> "") {?>
				<div class="input-group" id="detail-price">
					<span class="input-group-addon">판매단가</span>
					<?
					if($mem_no <> "")
					{	
					?>
						<span class="form-control"><strong id="sale_price" data-sale_price="<?=$rs_sale_price?>"><?=number_format($rs_sale_price)?></strong>원</span>
					<?
					}
					else
					{	
					...
					<?	}
						...
						{
					?>		<span class="form-control"><strong>가격문의</strong>
					<?
						}					
					}
					?>		
				...
				<div class="input-group" id="detail-piece">
					<span class="input-group-addon">주문 수량</span>
					<span class="form-control"><input type="button" value="<" onclick="js_change_goods_qty('-')"><input type="text" name="qty" style="width:50px;" value="<?=$rs_delivery_cnt_in_box?>" autocomplete="off"/><input type="button" value=">" onclick="js_change_goods_qty('+')">개</span>
				</div>
				<div class="input-group" id="detail-piece">
					<span class="input-group-addon">박스 입수</span>
					<span class="form-control"><strong><?=number_format($rs_delivery_cnt_in_box)?></strong>개</span>
				</div>
				<div class="input-group" id="detail-piece">
					<span class="input-group-addon">스티커 옵션</span>
					<span class="form-control"><?=makeSelectBoxForSticker($stickerData,'selectSticker');?></span>
				</div>
				<div class="input-group" id="detail-piece">
					<span class="input-group-addon">인쇄(통장지갑등)<br>
													추가비용</span>
				...
				<div class="input-group" id="detail-total">
				  <span class="input-group-addon">합계 금액</span>
					<?
					if($mem_no <> "")
					{	
					?>
						<span class="form-control"><strong id="total_sale_price"><?=number_format($rs_sale_price)?></strong>원</span>
					<?
					}
					else
					{	
				...
				<?
					...
					{	
					?>
						<button type="button" class="btn btn-default" id="btn-order"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> 주문하기</button>
						<button type="button" class="btn btn-default" id="btn-cart"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> 장바구니</button>
					<!--<button type="button" class="btn btn-default" id="btn-ask">견적서 출력</button>-->
					<?
					}
					else
					{	
					...
					<?	}
						...
						{
					?>		
						<button type="button" class="btn btn-default" id="btn-order" disabled><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> 주문하기</button>
						<button type="button" class="btn btn-default" id="btn-cart" disabled><span class="glyphicon glyphicon-ok" aria-hidden="true"></span> 장바구니</button>
					<?
						}					
					}
					?>
				...
				<!-- <div class="input-group" id="detail-price">
					<span class="input-group-addon">판매단가</span>
					<span class="form-control">문의요망</span>
				</div> -->
				<? //} ?>
				<!--
				<div class="input-group" id="detail-package">
				  <span class="input-group-addon">포장옵션</span>
				  <div class="btn-group">
					  <button type="button" class="btn btn-sm btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">종이 케이스 포함<span class="caret"></span></button>
					  <ul class="dropdown-menu">
						  <li role="presentation"><a href="#">종이 케이스 포함</a></li>
					  </ul>
				  </div>
				</div>
