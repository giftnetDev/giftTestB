				...
				<table cellpadding="0" cellspacing="0" class="rowstable fixed_header_table">
					...
					?>
						<tr class="<?=$str_use_style ?>" height="35" >
							...
							<td><?= $SALE_ADM_NM ?></td>
							<?
							$ledger = mysql_fetch_array(SumCompanyLedger($conn, $CP_NO));
							
							if($ledger["SUM_BALANCE"] < 0){
								$misu = number_format($ledger["SUM_BALANCE"]);
								$machulmisu = 0;
							} else {
								$misu = 0;
								$machulmisu = number_format($ledger["SUM_BALANCE"]);
							}
							

							// $misu = number_format(cal_misu($conn, $CP_NO, $end_date));
							// if($misu>0) $misu = 0;
							?>
							<td><a href="javascript:js_view_account_receivable_list('<?=$CP_CODE?>')"><?=$misu;?></a></td>
							<td><a href="javascript:js_view_account_receivable_report('<?=$CP_CODE?>')"><?=$machulmisu;?></a></td>
