	...
	if ($mode == "I") {
		...
		if (sizeof($arr_cp_no) > 0) {
			...
			for ($m = 0; $m < sizeof($arr_cp_no); $m++) {
				...
				//$result = insertGoodsPriceUpdate($conn, "TBL_GOODS_PRICE", $goods_no, $arr_cp_no[$m], "0", $new_sale_price, "0", $s_adm_no);
				
				//상품디테일 아래 상품가격 변동시
				$result = insertGoodsPriceUpdate($conn, "TBL_GOODS_PRICE", $goods_no, $arr_cp_no[$m], $price, $buy_price, $new_sale_price, $sticker_price, $print_price, $delivery_price, $delivery_cnt_in_box, $labor_price, $other_price, $sale_susu, $s_adm_no);
...
<script language="javascript">
		...
		opener.location.href = "goods_price_list.php?1=1<?=$strParam?>";
		self.close();
</script>
<?
		exit;
...
<input type="hidden" name="delivery_cnt_in_box" value="<?=$rs_delivery_cnt_in_box?>">
									...
									<tr>
										<td width="230" style="padding: 10px 10px 10px 0px">
											...
											* 업체 리스트 <br>
												...
												<?
													...
													if (sizeof($arr_company_no) > 0) {
														...
														for ($j = 0 ; $j < sizeof($arr_company_no); $j++) {
															...
															$CP_NM2					= trim($arr_company_no[$j]["CP_NM2"]);
															$CP_CODE				= trim($arr_company_no[$j]["CP_CODE"]);
												?>
												<option value="<?=$CP_NO?>"><?=$CP_NM." ".$CP_NM2."[".$CP_CODE."]"?></option>
												<?
														}
													}
												?>
											...
											<br/><br/>
											<input type="text" class="search_company" placeholder="업체명검색" value="" />
											<script>
												$(function(){
													var arr_full_options = $("select[name=sel_left] option").clone();
													$(".search_company").keyup(function(){
														
														if($(this).val() != "") { 
															$("select[name=sel_left]").children().remove().end();
															for (i = 0; i < arr_full_options.length; i++) {
																var cp_nm = arr_full_options[i].text;
																if(cp_nm.contains($(this).val()))
																	$("select[name=sel_left]").append(arr_full_options[i]);
															}

														} else {
															$("select[name=sel_left]").children().remove().end().append(arr_full_options);
														}
