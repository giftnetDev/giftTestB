...
#===============================================================
	...
	if(sizeof($arr_rs)>0){
		for($i=0;$i<sizeof($arr_rs);$i++){
			...
			$arr_rs[$i]["MRO_SALE_PRICE2"] = getMroSalePrice($conn, $TEMP_GOODS_NO);
		}
	}
			...
			<div class="sp20"></div>
				...
				<?
					...
					if (sizeof($arr_rs) > 0) {
						...
						for ($j = 0 ; $j < sizeof($arr_rs); $j++) {
							...
							$MRO_SALE_PRICE2		= trim($arr_rs[$j]["MRO_SALE_PRICE2"]);
				...
				?>
						...
						<tr class="<?=$str_use_style?> <?=$str_wrong_style?>" >
							...
							<td class="price" title="MRO판매가:<?=$MRO_SALE_PRICE2?>"><?= number_format($MRO_SALE_PRICE2) ?> 원</td>
						...
						<tr class="<?=$str_use_style?> <?=$str_wrong_style?>" >
							...
							<td class="modeual_nm"> <input type="button" name="b" value="조회" class="btntxt" onclick="js_link_to_item_ledger('<?=$GOODS_NO?>', '<?=$REG_DATE?>');"></td>
