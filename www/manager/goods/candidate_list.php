...
#=====================================================================
	...
	if ($mode == "M") {
		...
		for ($k = 0; $k < $row_cnt; $k++) {
			...
			$result = moveGoods($conn, $str_goods_no, $s_adm_no);
...
<script>
  ...
  <script type="text/javascript" >
	...
	function js_write() {
		document.location.href = "candidate_write.php";
	}
	...
	function js_view(rn, goods_no) {
		...
		frm.target = "";
		frm.method = "get";
		frm.action = "candidate_write.php";
		frm.submit();
	...
	*/
	...
	function js_move() {
		...
		bDelOK = confirm('선택한 상품을 실 상품으로 이동 하시겠습니까?');
					...
					<tr>
						...
						<td>
							...
							<input type="text" value="<?=$end_price?>" name="end_price" style="width: 75px;" class="txt" onkeyup="return isNumber(this)" /> 원
							 ...
							 (<input type="checkbox" name="chk_vendor" <? if($chk_vendor == "Y") echo "checked"; ?> value="Y"/> 벤더할인<input type="text" name="vendor_calc" value="<?=$vendor_calc?>" class="txt" style="width:25px;"/>%)
			...
			<div class="sp20"></div>
				...
				<?
					...
					if (sizeof($arr_rs) > 0) {
						...
						for ($j = 0 ; $j < sizeof($arr_rs); $j++) {
							...
							if($RESTOCK_DATE <> '0000-00-00 00:00:00')
				...
				?>
						...
						<tr class="<?=$str_use_style?> <?=$str_wrong_style?>" >
							...
							<td class="price">
								<? if ($chk_vendor == "Y") {?>
								<?= number_format($SALE_PRICE) ?> 원<br/><b><?= number_format($VENDER_PRICE)?> 원</b>
								<? } else { ?>
							...
							<td class="price"><?= $DELIVERY_CNT_IN_BOX ?></td>
				...
				<div style="width: 95%; text-align: right; margin: 10px 0 20px 0;">
					<? if ($sPageRight_D == "Y") {?>
						<input type="button" name="aa" value=" 실 상품으로 이동 " class="btntxt" onclick="js_move();">
				...
				-->
					...
					<?
						...
						if (sizeof($arr_rs) > 0) {
							...
							$strParam = $strParam."&nPageSize=".$nPageSize."&search_field=".$search_field."&search_str=".$search_str."&order_field=".$order_field."&order_str=".$order_str;
							$strParam = $strParam."&start_date=".$start_date."&end_date=".$end_date."&start_price=".$start_price."&end_price=".$end_price."&con_cate=".$con_cate;
							$strParam = $strParam."&con_cate_01=".$con_cate_01."&con_cate_02=".$con_cate_02."&con_cate_03=".$con_cate_03."&con_cate_04=".$con_cate_04."&con_tax_tf=".$con_tax_tf."&chk_vendor=".$chk_vendor."&vendor_calc=".$vendor_calc."&view_type=".$view_type."&exclude_category=".$exclude_category;
	...
	<script>
		...
		$(function(){
			...
			*/
			...
			$("input[name=vendor_calc]").keyup(function(){
				if($(this).val() != "")
