...
<script type="text/javascript" >
	...
	function js_write() {
		...
		var url = "/manager/category/pcode_write_popup.php";
	...
	function js_view(rn, seq) {
		...
		var url = "/manager/category/pcode_write_popup.php?mode=S&pcode_no="+seq;
		NewWindow(url, '대분류조회', '600', '353', 'NO');
	}
	
	function js_view_dcode(rn, seq) {

		var url = "/manager/category/dcode_list_popup.php?mode=R&pcode_no="+seq;
		NewWindow(url, '세부분류조회', '600', '650', 'NO');
	}

	function js_open_category(goods_cate) { 
		var has_sub_cate = document.getElementById('has_sub_cate');
		var url = "pop_goods_category.php?has_sub_cate=" + (has_sub_cate.checked ? 'Y' : 'N') + "&goods_cate=" + goods_cate;
		...
		<td class="contentarea">
			...
			<div id="mwidthwrap">
				...
				<div class="btnrighttxt">
					<? if ($sPageRight_I == "Y") {?>
						<a href="javascript:NewWindow('/manager/category/pop_category_write.php', 'pop_add_menu', '560', '285', 'no');">대표분류등록</a>&nbsp;&nbsp;
					<? } ?>
					<? if ($sPageRight_U == "Y") {?>
						<a href="javascript:NewWindow('/manager/category/pop_category_order.php', 'pop_order_menu', '560', '470', 'no');">순서변경 </a>&nbsp;&nbsp;
					<? } ?>
				</div>
				<div class="category_choice">
					<?= makeCategorySelectBoxOnChange($conn, $con_cate, $exclude_category);?>
					<label><input type="checkbox" id="has_sub_cate" name="has_sub_cate" <?if($has_sub_cate == "Y") echo "checked";?> value="Y"/>하위 카테고리 포함</label>
				...
				<?
					...
					if (sizeof($arr_rs) > 0) {
						...
						for ($j = 0 ; $j < sizeof($arr_rs); $j++) {
							...
							$goods_cnt = cntSearchGoodsCategory($conn, $CATE_CD, $has_sub_cate);
				...
				?>
					<tr>
						<td class="modeual_nm"><a href="javascript:NewWindow('/manager/category/pop_category_write.php?mode=S&m_level=<?=$CATE_CD?>&cate_no=<?=$CATE_NO?>', 'pop_modify_menu', '560', '285', 'no');"><?=$cate_str?></a></td>
						<td class="modeual_nm"><a href="javascript:NewWindow('/manager/category/pop_category_write.php?mode=S&m_level=<?=$CATE_CD?>&cate_no=<?=$CATE_NO?>', 'pop_modify_menu', '560', '285', 'no');"><?= $CATE_CD ?></a></td>
						<td class="modeual_nm"><?=$CATE_MEMO?></td>
						<td><?=number_format($goods_cnt)?></td>
						<td>
							<a href="javascript:js_open_category('<?=$CATE_CD?>');">추가/삭제</a>&nbsp;&nbsp;
						</td>
						<td>
							<a href="/manager/goods/goods_list.php?con_cate=<?=$CATE_CD?>" target="_blank" style="text-decoration:underline;">상품관리로</a>
						</td>
						<td class="filedown">
							<? 
								if ($sPageRight_I == "Y") {
							...
							?>
							<? if ($sPageRight_I == "Y") {?>
							<a href="javascript:NewWindow('/manager/category/pop_category_write.php?m_level=<?=$CATE_CD?>&m_seq01=<?=$CATE_SEQ01?>&m_seq02=<?=$CATE_SEQ02?>&m_seq03=<?=$CATE_SEQ03?>', 'pop_add_menu', '560', '285', 'no');">대분류등록</a>&nbsp;
							<? } ?>
							<? if ($sPageRight_U == "Y") {?>
							<a href="javascript:NewWindow('/manager/category/pop_category_order.php?m_level=<?=$CATE_CD?>', 'pop_order_menu', '560', '470', 'no');">순서변경</a>
							<? } ?>
							<?
										} else if (strlen($CATE_CD) == 4){
							?>
							<? if ($sPageRight_I == "Y") {?>
							<a href="javascript:NewWindow('/manager/category/pop_category_write.php?m_level=<?=$CATE_CD?>&m_seq01=<?=$CATE_SEQ01?>&m_seq02=<?=$CATE_SEQ02?>&m_seq03=<?=$CATE_SEQ03?>', 'pop_add_menu', '560', '285', 'no');">중분류등록</a>&nbsp;
							<? } ?>
							<? if ($sPageRight_U == "Y") {?>
							<a href="javascript:NewWindow('/manager/category/pop_category_order.php?m_level=<?=$CATE_CD?>', 'pop_order_menu', '560', '470', 'no');">순서변경</a>
							<? } ?>
							...
							?>
							<? if ($sPageRight_I == "Y") {?>
							<a href="javascript:NewWindow('/manager/category/pop_category_write.php?m_level=<?=$CATE_CD?>&m_seq01=<?=$CATE_SEQ01?>&m_seq02=<?=$CATE_SEQ02?>&m_seq03=<?=$CATE_SEQ03?>', 'pop_add_menu', '560', '285', 'no');">소분류등록</a>&nbsp;
							<? } ?>
							<? if ($sPageRight_U == "Y") {?>
							<a href="javascript:NewWindow('/manager/category/pop_category_order.php?m_level=<?=$CATE_CD?>', 'pop_order_menu', '560', '470', 'no');">순서변경</a>
							<? } ?>
