...
#====================================================================
	...
	function getOriginalGoodsInfo(&$arrGoods, &$arrOriginal){


		$arrOriginal["GOODS_CATE"]					=	trim($arrGoods[0]["GOODS_CATE"]);
		$arrOriginal["GOODS_NAME"]					=	trim($arrGoods[0]["GOODS_NAME"]);
		$arrOriginal["GOODS_SUB_NAME"]				=	trim($arrGoods[0]["GOODS_SUB_NAME"]);
		$arrOriginal["GOODS_CODE"]					=	trim($arrGoods[0]["GOODS_CODE"]);
		$arrOriginal["MSTOCK_CNT"]					=	trim($arrGoods[0]["MSTOCK_CNT"]);
		$arrOriginal["CATE_04"]						=	trim($arrGoods[0]["CATE_04"]);
		$arrOriginal["TAX_TF"]						=	trim($arrGoods[0]["TAX_TF"]);
		$arrOriginal["MEMO"]						=	trim($arrGoods[0]["MEMO"]);
		$arrOriginal["USE_TF"]						=	trim($arrGoods[0]["USE_TF"]);
		$arrOriginal["EXPOSURE_TF"]					=	trim($arrGoods[0]["EXPOSURE_TF"]);
		$arrOriginal["CONCEAL_PRICE_TF"]			=	trim($arrGoods[0]["CONCEAL_PRICE_TF"]);

	}

	// function 

	function insertGoodsChangeLog(	$db, 
									$goods_cate, $goods_code, $goods_name, $goods_sub_name, 
									$cate_04, $mstock_cnt, $tax_tf,  
									$memo, 
									 $use_tf, $goods_no, $exposure_tf, $conceal_price_tf)
	{

		$query="SELECT 		GOODS_CATE, 	GOODS_NAME, 	GOODS_SUB_NAME, 	GOODS_CODE, 		MSTOCK_CNT, 
							CATE_04, 		TAX_TF, 		MEMO, 				USE_TF,				EXPOSURE_TF, 
							CONCEAL_PRICE_TF
				FROM 		TBL_GOODS 
		...
		if(!$result){
			echo "<script>alert('INSERT_GOODS_CHANGE_LOG-ERROR()');</scirpt>";
		}
		$oldGoodsInfo=mysql_fetch_row($result);

		{

			$OLD_GOODS_CATE			=$oldGoodsInfo[0];
			$OLD_GOODS_NAME			=$oldGoodsInfo[1];
			$OLD_GOODS_SUB_NAME		=$oldGoodsInfo[2];
			$OLD_GOODS_CODE			=$oldGoodsInfo[3];
			$OLD_MSTOCK_CNT			=$oldGoodsInfo[4];
			$OLD_CATE_04			=$oldGoodsInfo[5];
			$OLD_TAX_TF				=$oldGoodsInfo[6];
			$OLD_MEMO				=$oldGoodsInfo[7];
			$OLD_MEMO				=$oldGoodsInfo[7];
			$OLD_USE_TF				=$oldGoodsInfo[8];
			$OLD_EXPOSURE_TF		=$oldGoodsInfo[9];
			$OLD_CONCEAL_PRICE_TF	=$oldGoodsInfo[10];
		}
	/*
		$OLD_GOODS_NAME
		$OLD_GOODS_SUB_NAME
		...
		이제 OLD_GOODS_NAME,OLD_GOODS_SUB_NAME, OLD_MEMO도 /가 필요하다.
		/을 씌워주는 함수 SetStringToDB()

		아니면 반대로 goods_name, goods_sub_name, memo 에 /를 벗겨준다.
		/을 벗겨주는 함수 SetStringFromDB()



	*/

		$goods_name=SetStringFromDB($goods_name);
		$goods_sub_name=SetStringFromDB($goods_sub_name);
		$memo=SetSTringFromDB($memo);
		...
		if($goods_cate!=$OLD_GOODS_CATE){
			$idxQuery.=" GOODS_CATE, AFTER_GOODS_CATE,";
			$valueQuery.=" '$OLD_GOODS_CATE', '$goods_cate',";
			$insertTF='Y';

		}
		if($goods_name!=$OLD_GOODS_NAME){

			$goods_name=trim(SetStringToDB($goods_name));
			$OLD_GOODS_NAME=trim(SetStringToDB($OLD_GOODS_NAME));

			$idxQuery.=" GOODS_NAME, AFTER_GOODS_NAME,";
			$valueQuery.=" '$OLD_GOODS_NAME', '$goods_name',";
			$insertTF='Y';
		}
		if($goods_sub_name!=$OLD_GOODS_SUB_NAME){

			$goods_sub_name=trim(SetStringToDB($goods_sub_name));
			$OLD_GOODS_SUB_NAME=trim(SetStringToDB($OLD_GOODS_SUB_NAME));

			$idxQuery.=" GOODS_SUB_NAME, AFTER_GOODS_SUB_NAME,";
			$valueQuery.=" '$OLD_GOODS_SUB_NAME', '$goods_sub_name',";
			$insertTF='Y';
		}

		if($goods_code!=$OLD_GOODS_CODE){
			$idxQuery.=" GOODS_CODE, AFTER_GOODS_CODE,";
			$valueQuery.=" '$OLD_GOODS_CODE', '$goods_code',";
			$insertTF='Y';
		}
		if($mstock_cnt!=$OLD_MSTOCK_CNT){
			$idxQuery.=" MSTOCK_CNT, AFTER_MSTOCK_CNT,";
			$valueQuery.=" '$OLD_MSTOCK_CNT', '$mstock_cnt',";
			$insertTF='Y';
		}
		if($cate_04!=$OLD_CATE_04){

			$cate_04=trim(SetStringToDB($cate_04));
			$OLD_CATE_04=trim(SetStringToDB($OLD_CATE_04));

			$idxQuery.=" CATE_04, AFTER_CATE_04,";
			$valueQuery.=" '$OLD_CATE_04', '$cate_04',";
			$insertTF='Y';
		}
		if($tax_tf!=$OLD_TAX_TF){

			$tax_tf=trim(SetStringToDB($tax_tf));
			$OLD_TAX_TF=trim(SetStringToDB($OLD_TAX_TF));

			$idxQuery.=" TAX_TF, AFTER_TAX_TF,";
			$valueQuery.=" '$OLD_TAX_TF', '$tax_tf',";
			$insertTF='Y';
		}
		if(trim($memo)!=trim($OLD_MEMO)){

			$memo=trim(SetStringToDB($memo));
			$OLD_MEMO=trim(SetStringToDB($OLD_MEMO));


			$idxQuery.=" MEMO, AFTER_MEMO,";
			$valueQuery.=" '$OLD_MEMO', '$memo',";
			$insertTF='Y';
		}
		if($use_tf!=$OLD_USE_TF){

			$use_tf=trim(SetStringToDB($use_tf));
			$OLD_USE_TF=trim(SetStringToDB($OLD_USE_TF));

			$idxQuery.=" USE_TF, AFTER_USE_TF,";
			$valueQuery.=" '$OLD_USE_TF', '$use_tf',";
			$insertTF='Y';
		}
		if($exposure_tf!=$OLD_EXPOSURE_TF){

			$exposure_tf=trim(SetStringToDB($exposure_tf));
			$OLD_EXPOSURE_TF=trim(SetStringToDB($OLD_EXPOSURE_TF));

			$idxQuery.=" EXPOSURE_TF, AFTER_EXPOSURE_TF,";
			$valueQuery.=" '$OLD_EXPOSURE_TF', '$exposure_tf',";
			$insertTF='Y';
		}
		if($conceal_price_tf!=$OLD_CONCEAL_PRICE_TF){

			$conceal_price_tf=trim(SetStringToDB($conceal_price_tf));
			$OLD_CONCEAL_PRICE_TF= trim(SetStringToDB($OLD_CONCEAL_PRICE_TF));

			$idxQuery.=" CONCEAL_PRICE_TF, AFTER_CONCEAL_PRICE_TF,";
			$valueQuery.=" '$OLD_CONCEAL_PRICE_TF', '$conceal_price_tf',";
			$insertTF='Y';
		}

		if($insertTF=="Y"){

			$regAdm=$_SESSION['s_adm_no'];
			$idxQuery.=" REG_ADM, REG_DATE, GOODS_NO ";
			$valueQuery.=" '$regAdm', NOW(), '$goods_no' ";

			$insertQuery="INSERT INTO TBL_GOODS_CHANGE_LOG($idxQuery) VALUES($valueQuery) ; ";

			//  echo $insertQuery."<br>";
			// exit;

			$result=mysql_query($insertQuery, $db);
			if(!$result){
				echo"<script>alert('INSERT:GOODS_CHANGE_LOG-ERROR');</script>";
				exit;
			}
...
//		$goods_no	= $new_goods_no;
	...
	if ($mode == "U") {
		...
		insertGoodsChangeLog($conn,$goods_cate, $goods_code, $goods_name,$goods_sub_name, $cate_04, $mstock_cnt, $tax_tf, $memo, $use_tf, $goods_no,$exposure_tf, $conceal_price_tf);
		...
		uploadEx($_FILES[urlDetail], $fileDir, $goods_code,20, array('gif','jpeg','jpg','png'));
		...
		$result = updateGoods($conn, $goods_cate, $goods_code, $goods_name, $goods_sub_name, $cate_01, $cate_02, $cate_03, $cate_04, $restock_date, $price, $buy_price, $sale_price, 
								$next_sale_price, $extra_price, $stock_tf, $stock_cnt, $mstock_cnt, $tax_tf, $img_url, $file_nm_100, $file_rnm_100, $file_path_100, $file_size_100, $file_ext_100, 
								$file_nm_150, $file_rnm_150, $file_path_150, $file_size_150, $file_ext_150, $contents, $memo, $delivery_cnt_in_box, $sticker_price, $print_price, $delivery_price, 
								$sale_susu, $labor_price, $other_price, $use_tf, $s_adm_no, $goods_no, $exposure_tf, $conceal_price_tf);
	...
	if ($mode == "S") {
		...
		$rs_conceal_price_tf	= trim($arr_rs[0]["CONCEAL_PRICE_TF"]);		//202106008 추가
		...
		getOriginalGoodsInfo($arr_rs, $arrOriginal);
	}
...
<script type="text/javascript">
	...
	function js_save() {
		...
		// js_compare_to_before_and_after()
...
<script type="text/javascript">
	...
	function js_calculate_buy_and_sale_price( )	{
		...
		if ($("input[name=buy_price]").val() != ""){
			i_buy_price = parseInt($("input[name=buy_price]").val());
		} 
...
$(document).ready(function(){
	$("input:radio[name='rs_use_tf']").change(function(){
		var tempRadioValue=$("input:radio[name='rs_use_tf']:checked").val();
		alert(tempRadioValue);
		
		// return;
	});

	$("#chkExposureTF").change(function(){
		var exposureTF='A';
		var goodNo="<?=$goods_no?>";
		if($("#chkExposureTF").is(":checked")){
			//alert("chk됨");
			exposureTF='Y';
		}
		else{
		...
		$("input[name='exposure_tf']").val(exposureTF);
	});
	
	$("input[name='chkConcealTF']").change(function(){		//20210608 배너에서 가격문의로 변경
		var concealTF="";
		if($("input[name='chkConcealTF']").is(":checked")){
			concealTF="Y";
		}
		else{
			concealTF="N";
		}
		$("input[name='conceal_price_tf']").val(concealTF);
			...
			<h2>상품 관리</h2>
				...
				<tbody>
					...
					<tr>
						...
						<th style="color:blue">전시여부</th>

						<td class="line">전시함 &nbsp;<input type="checkbox" id='chkExposureTF' <?if($rs_exposure_tf=="Y") echo "checked";?>></td>
						<input type="hidden" name="exposure_tf" value="<?=$rs_exposure_tf?>">
					</tr>
					<tr>
						<th>가격문의</th><!--20210608 배너에서 가격문의로 변경-->						
						<td>&nbsp;&nbsp;&nbsp;<input type='checkbox' name="chkConcealTF" <?if($rs_conceal_price_tf=="Y") echo "checked";?>></td>	
						<input type="hidden" name="conceal_price_tf" value="<?=$rs_conceal_price_tf?>">
						
						<th></th>
						<td class="line"><input type="checkbox" id='chkConfirmTF'></td>
					</tr>
				</table>
				...
				<div id="tabs" style="width:95%; margin:10px 0;">
					<ul>
						...
						<? if ($goods_no != "") { ?>					<!--20210413 추가-->
						<li><a href="#tabs-5">스티커 옵션</a></li>		
						<? } ?>
					</ul>
					<div id="tabs-1">
						...
						* 다음 전단지 가격 
						...
						<colgroup>
							...
							<tr>
								<th>다음 적용 판매가</th>
								<td class="line" colspan="3">
									<input type="text" name="next_sale_price" value="<?=$rs_next_sale_price?>"/ disabled>  원	<!--20210805 DISABLED 처리함-->
								</td>
							</tr>
						</table>
						<div class="sp20"></div>
						...
						?>
							<? if($j == 0) {  ?>
								<tr>
									...
									<?if($CURRENT_REASEON == ""){?>
									<td class="filedown wordbreak" id="reason_<?=$goods_no?>"><a href="javascript:modifyCurrentReason('<?=$goods_no?>');">추가</a></td>
									<?} else {?>
									<td class="filedown wordbreak" id="reason_<?=$goods_no?>"><a href="javascript:modifyCurrentReason('<?=$goods_no?>');"><?=$CURRENT_REASEON?></a></td>
									<?}?>
								</tr>
							<? } ?>
								<tr class="<?=$str_price_class?>">
									...
									<? } ?>
									<?if($REASON==""){?>
										<td class="filedown wordbreak" id="pre_reason_<?=$PRE_SEQ_NO?>"><a href="javascript:modifyReason('<?=$PRE_SEQ_NO?>');">추가</a></td>
									<?} else {?>
										<td class="filedown wordbreak" id="pre_reason_<?=$PRE_SEQ_NO?>"><a href="javascript:modifyReason('<?=$PRE_SEQ_NO?>');"><?=$REASON?></a></td>
									<?}?>
								</tr>
						<?			
						...
						<table cellpadding="0" cellspacing="0" class="colstable02">
							...
							<tr>
								...
								<td colspan="3" class="subject line">
									<input type="file" name="urlDetail" class="txt"><br>
									<?
					...
					<? if ($goods_no != "") { ?>					<!--20210413 추가-->
						<div id="tabs-5">		
							<iframe name="stikerOption" src="/manager/goods/goods_option_stiker.php?goods_no=<?=$goods_no?>" frameborder="no" width="95%" height="500px" marginwidth="0" marginheight="0"></iframe>
						</div>
					<? } ?>
