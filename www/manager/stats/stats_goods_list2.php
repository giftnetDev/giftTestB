...
#==============================================================================
	...
	if ($code == "company") {
		$str_menu_title = "업체별 판매 현황";

	}

	if ($code == "period") {
		$str_menu_title = "기간별 판매 현황";
...
#===============================================================
	...
	$nListCnt = totalCntStatsByCompanyLedger($conn, $code, $con_cate, $start_date, $end_date, $cp_type2, $sel_opt_manager_no, $search_field, $search_str);
	...
	$arr_rs = listStatsByCompanyLedger($conn, $code, $con_cate, $start_date, $end_date, $cp_type2, $sel_opt_manager_no, $search_field, $search_str, $order_field, $order_str, $nPage, $nPageSize, $nListCnt);
...
<script language="javascript">
    ...
    function js_view_ledger(code) { 
		...
		<? if ($code == "goods") {?>
		window.open("/manager/confirm/ledger_list.php?sel_inout_type=매출&search_date_type=ledger_date&start_date="+frm.start_date.value+"&end_date="+frm.end_date.value+"&search_field=NAME&search_str=" + code + "&cp_type=" + frm.cp_type2.value,'_blank');
		<? } ?>

		<? if ($code == "company") {?>
		window.open("/manager/confirm/ledger_list.php?sel_inout_type=매출&search_date_type=ledger_date&start_date="+frm.start_date.value+"&end_date="+frm.end_date.value+"&search_field=NAME&search_str=" + frm.search_str.value + "&cp_type=" + code,'_blank');
		<? } ?>

		<? if ($code == "period") {?>
		window.open("/manager/confirm/ledger_list.php?sel_inout_type=매출&search_date_type=ledger_date&start_date="+code+"-01&end_date="+code+"-31&search_field=NAME&search_str=" + frm.search_str.value + "&cp_type=" + frm.cp_type2.value,'_blank');
		<? } ?>
	}
		...
		<td class="contentarea">
			...
			<div id="mwidthwrap">
				...
				<div style="text-align:right; width:95%">
					*. 원장 기준으로 통계가 작성 되었기 때문에 최초 주문확인일 = 기장일 입니다. 클레임으로 인한 반품,교환,취소는 제외, 증정/샘플/추가 제외
				</div>
					...
					<tr>
						...
						<td>
							<select name="order_field" style="width:144px;">
								<option value="ORDER_TOTAL" <? if ($order_field == "ORDER_TOTAL") echo "selected"; ?> >주문회수</option>
								<option value="QTY_TOTAL" <? if ($order_field == "QTY_TOTAL") echo "selected"; ?> >주문수량</option>
				...
				<?
					...
					if (sizeof($arr_rs) > 0) {
						for ($j = 0 ; $j < sizeof($arr_rs); $j++) {
							...
							$SALE_STATE					= trim($arr_rs[$j]["SALE_STATE"]);
						...
						<tr height="37">
							...
							<td class="modeual_nm">
								<?
									if($code == "goods") { 
								?>
								<a href="javascript:js_view_ledger('[<?=$CODE?>]')"><?=$CODE?></a>
								<?
									}
								?>
								<?
							...
							<td class="modeual_nm">
								<?
									if($code == "goods") { 
								?>
								<a href="javascript:js_view_ledger('[<?=$CODE?>]')"><?=$TITLE?></a>
								...
								<a href="/manager/company/company_list.php?search_field=CP_CODE&search_str=<?=$CODE?>" target="_blank" style="font-weight:bold; font-size:10px;">(보기)</a>
								<?
									}
								?>
								<?
