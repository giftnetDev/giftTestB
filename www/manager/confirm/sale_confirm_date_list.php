...
function js_toggle() {
	...
	function js_detail(confirm_ymd,cp_no,cp_nm,account_bank,account,ad_type,cp_phone) {
		var frm = document.frm;

		frm.p_confirm_ymd.value		= confirm_ymd;
		frm.p_cp_no.value					= cp_no;
		...
		NewWindow('about:blank', 'sale_order_detail_list','1018','600','YES');
		
		frm.target = "sale_order_detail_list";
		frm.action = "sale_confirm_order_detail_list.php";
		frm.submit();
...
<input type="hidden" name="p_cp_no" value="">
					...
					<tr>
						...
						<td>
							<select name="order_field" style="width:84px;">
								<option value="AA.SALE_CONFIRM_YMD" <? if ($order_field == "AA.SALE_CONFIRM_YMD") echo "selected"; ?> >정산일</option>
			...
			<table cellpadding="0" cellspacing="0" class="rowstable" border="0">
				...
				<?
					...
					if (sizeof($arr_rs) > 0) {
						...
						?>
						<tr>
							<td><a href="javascript:js_detail('<?=$CONFIRM_YMD?>','<?=$CP_NO?>','<?=$CP_NM?>','<?=$ACCOUNT_BANK?>','<?=$ACCOUNT?>','<?=$AD_TYPE?>','<?=$CP_PHONE?>');"><?=$CONFIRM_YMD?></a></td>
							<td class="modeual_nm"><?=$AD_TYPE?></td>
							<td class="modeual_nm"><a href="javascript:js_detail('<?=$CONFIRM_YMD?>','<?=$CP_NO?>','<?=$CP_NM?>','<?=$ACCOUNT_BANK?>','<?=$ACCOUNT?>','<?=$AD_TYPE?>','<?=$CP_PHONE?>');"><?=$CP_NM?></a></td>
							<td class="filedown"><?=$ACCOUNT_BANK?></td>
							<td class="modeual_nm"><?=$ACCOUNT?></td>
							<td class="modeual_nm"><?=$CP_PHONE?></td>
