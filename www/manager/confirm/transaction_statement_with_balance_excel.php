...
#===============================================================
	...
	$arr_rs_ledger = selectBaseLedgerByOrder($conn, $rs_cp_no, $reserve_no);
	if(sizeof($arr_rs_ledger) > 0) { 

		//INOUT_DATE, REG_DATE, ORDER_GOODS_NO, CL_NO 

		$base_inout_date	  = trim($arr_rs_ledger[0]["INOUT_DATE"]);
		$base_reg_date		  = trim($arr_rs_ledger[0]["REG_DATE"]);
		$base_order_goods_no  = trim($arr_rs_ledger[0]["ORDER_GOODS_NO"]);
		$base_cl_no	  		  = trim($arr_rs_ledger[0]["CL_NO"]);


		$PREV_BALANCE		  = getPrevBalance($conn, $rs_cp_no, $base_inout_date, $base_order_goods_no, $base_cl_no);
		$SUM_SALES			  = $TOTAL_SALE_PRICE;
		$SUM_COLLECT		  = getSumCollect($conn, $rs_cp_no, $base_inout_date);
		$SUM_BALANCE		  = $PREV_BALANCE + $SUM_SALES - $SUM_COLLECT;
	} 
	...
	//12-15열 
	$sheetIndex->setCellValue("A12", iconv("EUC-KR", "UTF-8","전잔액"));
	$sheetIndex->mergeCells("A12:J12");
	$sheetIndex->setCellValue("K12", iconv("EUC-KR", "UTF-8", number_format($PREV_BALANCE)));
	$sheetIndex->mergeCells("K12:S12");
	$sheetIndex->setCellValue("A13", iconv("EUC-KR", "UTF-8","출고액"));
	$sheetIndex->mergeCells("A13:J13");
	$sheetIndex->setCellValue("K13", iconv("EUC-KR", "UTF-8", number_format($SUM_SALES)));
	$sheetIndex->mergeCells("K13:S13");
	$sheetIndex->setCellValue("A14", iconv("EUC-KR", "UTF-8","입금액"));
	$sheetIndex->mergeCells("A14:J14");
	$sheetIndex->setCellValue("K14", iconv("EUC-KR", "UTF-8", number_format($SUM_COLLECT)));
	$sheetIndex->mergeCells("K14:S14");
	$sheetIndex->setCellValue("A15", iconv("EUC-KR", "UTF-8","잔 액"));
	$sheetIndex->mergeCells("A15:J15");
	$sheetIndex->setCellValue("K15", iconv("EUC-KR", "UTF-8", number_format($SUM_BALANCE)));
	$sheetIndex->mergeCells("K15:S15");
	$sheetIndex->mergeCells("T12:AF15");

	$sheetIndex->getRowDimension(12)->setRowHeight(22.50);
	$sheetIndex->getRowDimension(13)->setRowHeight(22.50);
	$sheetIndex->getRowDimension(14)->setRowHeight(22.50);
	$sheetIndex->getRowDimension(15)->setRowHeight(22.50);
	
	//16열
	//월	일	품목	규격	수량	단가	공급가액 세액			
	$sheetIndex->setCellValue("A16", iconv("EUC-KR", "UTF-8","월"));
	$sheetIndex->setCellValue("B16", iconv("EUC-KR", "UTF-8","일"));
	$sheetIndex->setCellValue("C16", iconv("EUC-KR", "UTF-8","품    목"));
	$sheetIndex->mergeCells("C16:J16");
	$sheetIndex->setCellValue("K16", iconv("EUC-KR", "UTF-8","규격"));
	$sheetIndex->mergeCells("K16:O16");
	$sheetIndex->setCellValue("P16", iconv("EUC-KR", "UTF-8","수량"));
	$sheetIndex->mergeCells("P16:S16");
	$sheetIndex->setCellValue("T16", iconv("EUC-KR", "UTF-8","단가"));
	$sheetIndex->mergeCells("T16:W16");
	$sheetIndex->setCellValue("X16", iconv("EUC-KR", "UTF-8","공급가액"));
	$sheetIndex->mergeCells("X16:AB16");
	$sheetIndex->setCellValue("AC16", iconv("EUC-KR", "UTF-8","세액"));
	$sheetIndex->mergeCells("AC16:AF16");
	...
	$sheetIndex->getStyle("A12:AF15")->applyFromArray($BStyle_middle);
	$sheetIndex->getStyle("A17:AF".($k-1))->applyFromArray($BStyle_middle);
	...
	$sheetIndex->getStyle("A12:S15")->getFont()->setColor($fColor);
	$sheetIndex->getStyle("A17:AF28")->getFont()->setColor($fColor);
	...
	// Rename sheet
	$sheetIndex->setTitle(iconv("EUC-KR", "UTF-8","거래명세표"));

	// Set active sheet index to the first sheet, so Excel opens this as the first sheet
	$objPHPExcel->setActiveSheetIndex(0);

	// 파일의 저장형식이 utf-8일 경우 한글파일 이름은 깨지므로 euc-kr로 변환해준다.
	$filename = "거래명세표-".$SENDER_NM."-".$reserve_no."-".date("Ymd");
