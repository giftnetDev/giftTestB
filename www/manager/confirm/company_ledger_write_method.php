...
#====================================================================
	...
	if ($mode == "I") {
		...
		switch($inout_method){ 
			...
			case "기타":
						if($inout_type=="입금"){
							$inout_type_code="RO09";
							$inout_type="기타입금";
						}
						else{
							$inout_type_code="LO10";
							$inout_type="기타출금";
						}
						$name=getCompanyName($conn, $cp_type);
						$to_cp_no=null;
						$sertax = 0;
						$result = insertCompanyLedger($conn, $cp_type, $inout_date, $inout_type_code, $goods_no, $name, $qty, $unit_price, $to_cp_no, $surtax, "", "", $memo, $reserve_no, $order_goods_no, "기타", $reg_no, $s_adm_no, null);
						break;
			default:
						if($inout_type == "입금") 
						...
						if($chkAnotherMethod  && $inout_type="입금"){
							$name=trim(SetStringToDB($txtAnotherMethod));
							$inout_type_code="RR03";
							$memo="Ex";
...
<script language="javascript">
	...
	function js_save() {
		...
		var inout=frm.inout_method.value;
		var comment=frm.memo.value;
		...
		<td class="contentarea">
			...
			<div id="mwidthwrap">
				...
				<table cellpadding="0" cellspacing="0" class="colstable">
					...
					<tr height="40">
						<th rowspan="6">
							...
							<td>
								...
								&nbsp;&nbsp;
								다른 이름<input type="checkbox" name='chkAnotherMethod'>
								&nbsp;&nbsp;
								<input type="text" name="txtAnotherMethod">
							</td>
						<th>업체구분</th>
							<td>
								<span id="cptype"></span>
					...
					<tr height="40">
						<td colspan="3">
							<label><input type='radio' name='inout_method' value='기타'> 기타 </label>
							&nbsp;&nbsp;&nbsp;
						</td>
					</tr>
					<tr height="40">
