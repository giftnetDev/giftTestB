...
<script language="javascript">
	...
	function js_write() {
		document.location.href = "event_write.php";
	}
...
?>
      ...
      <div id="mwidthwrap">
        ...
        <div class="category_choice">
					<?= makeSelectBoxOnChange($conn,"STATE","con_event_state","125","상태선택","",$con_event_state)?>&nbsp;
					
					<?= makeSelectBoxOnChange($conn,"USE_TF","con_use_tf","155","공개 여부 선택","",$con_use_tf)?>&nbsp;
				</div>     
							...
							<?
								...
								if (sizeof($arr_rs) > 0) {
									...
									for ($j = 0 ; $j < sizeof($arr_rs); $j++) {
										...
										$EVENT_STATE		= trim($arr_rs[$j]["EVENT__STATE"]);
        ...
        <tr> 
          ...
          <td class="filedown"><?=$str_state?></td>
					<td><a href="javascript:js_toggle('<?=$EVENT_NO?>','<?=$USE_TF?>');"><?= $STR_USE_TF ?></a></td>
          <td><?= $REG_DATE ?></td>
        </tr>
							<?			
									}
