...
# Confirm right
#==============================================================================
	$menu_right = "SG002"; // 메뉴마다 셋팅 해 주어야 합니다
...
#====================================================================
	...
	#====================================================================
		...
		$file_dir = $HTTP_SERVER_VARS["DOCUMENT_ROOT"]."/upload_data/temp_order/".$file_nm;
		...
		while($str = fgets($fo, 3000)){
			...
			$buy_cp_no			= trim($a_str[0]);
			$goods_no				= trim($a_str[1]);
			$goods_name			= trim($a_str[2]);
			$buy_price			= trim($a_str[3]);
			$qty						= trim($a_str[4]);
...
<script language="javascript">
	...
	function js_view(rn, file_nm, order_no) {
		...
		NewWindow(url, '입고등록입력', '800', '420', 'YES');
