...
#====================================================================
	...
	if ($mode == "I") {
		...
		if (isset($arr_option_name)) 
		{
			...
			$RS_CATE_01								= SetStringFromDB($arr_rs[0]["CATE_01"]); 
			$RS_TAX_TF								= SetStringFromDB($arr_rs[0]["TAX_TF"]); 
			...
			for($j = 0; $j < sizeof($arr_option_name); $j ++) { 
				...
				if($t_option_nm != "" && $t_option_qty != "" && $t_option_price != "") { 
					...
					$result2 = insertCompanyLedger($conn, $RS_CP_NO, $RS_INOUT_DATE, $RS_INOUT_TYPE, null, $t_option_nm, $t_option_qty, $t_option_price, $withdraw, $RS_DEPOSIT, $RS_CATE_01, $RS_TAX_TF, $RS_MEMO, $RS_RESERVE_NO, $RS_ORDER_GOODS_NO, $RS_RGN_NO, $s_adm_no);
