...
#====================================================================
		...
		$new_bb_no =  insertBoard($conn, $bb_code, $cate_01, $cate_02, $cate_03, $cate_04, $writer_nm, $writer_pw, $email, $homepage, $title, $ref_ip, $recomm, htmlspecialchars($contents), $file_nm, $file_rnm, $file_path, $file_size, $file_ext, $keyword, $comment_tf, $use_tf, $s_adm_no);
		
		$result = deleteBoardCompany($conn, $bb_code, $new_bb_no);
		...
		if (sizeof($arr_cp_no) > 0) {
			...
			for ($m = 0; $m < sizeof($arr_cp_no); $m++) {
				$result = insertBoardCompany($conn, $bb_code, $new_bb_no, $arr_cp_no[$m]);
			}
		}
...
#====================================================================
		...
		$result = updateBoard($conn, $cate_01, $cate_02, $cate_03, $cate_04, $writer_nm, $writer_pw, $email, $homepage, $title, $ref_ip, $recomm, htmlspecialchars($contents), $file_nm, $file_rnm, $file_path, $file_size, $file_ext, $keyword, $comment_tf, $use_tf, $s_adm_no, $bb_code, $bb_no);

		$result = deleteBoardCompany($conn, $bb_code, $bb_no);
		...
		if (sizeof($arr_cp_no) > 0) {
			...
			for ($m = 0; $m < sizeof($arr_cp_no); $m++) {
				$result = insertBoardCompany($conn, $bb_code, $bb_no, $arr_cp_no[$m]);
			}
		}
	}
...
<script language="javascript">
		alert('정상 처리 되었습니다.');
		document.location.href = "company_notice_list.php<?=$strParam?>";
</script>
<?
		exit;
	}	
...
function js_list() {
	...
	frm.contents.value = "";
	frm.method = "post";
	frm.action = "company_notice_list.php";
	frm.submit();
}
...
function js_select_company() {
	...
	if (type == "전체") {	
		for (k = 0; k < arr_company_no.length; k++) {
			objLeft.options[objLeft.length] = new Option(arr_company_name[k],  arr_company_no[k]);
		}
	}

	if (type == "판매") {	
		for (k = 0; k < arr_company_no.length; k++) {
			if (arr_company_type[k].indexOf("판매") > -1) {
				objLeft.options[objLeft.length] = new Option(arr_company_name[k],  arr_company_no[k]);
			}
		}
	}
	...
	if (type == "운영") {	
		for (k = 0; k < arr_company_no.length; k++) {
			if (arr_company_type[k].indexOf("운영") > -1) {
				objLeft.options[objLeft.length] = new Option(arr_company_name[k],  arr_company_no[k]);
			}
		}
	}
	...
	if (type == "구매") {	
		for (k = 0; k < arr_company_no.length; k++) {
			if (arr_company_type[k].indexOf("구매") > -1) {
				objLeft.options[objLeft.length] = new Option(arr_company_name[k],  arr_company_no[k]);
			}
		}
	}
...
<?= makeCompanyScriptArray($conn, "arr_company"); ?>
				...
				<table border="0">
					<tr>
						<td width="230" style="padding: 10px 10px 10px 0px">
							...
							<input type="radio" value="전체" name="chk_sale" onClick="js_select_company();" checked> 전체 
							<input type="radio" value="운영" name="chk_sale" onClick="js_select_company();"> 운영업체 <br/>
							<input type="radio" value="판매" name="chk_sale" onClick="js_select_company();"> 판매업체 
							<input type="radio" value="구매" name="chk_sale" onClick="js_select_company();"> 공급업체 
							...
							* 업체 리스트 <br>
							<select name="sel_left" size="10" style="width:230px" multiple>
								<?
									$arr_company_no = listCompanyBoard($conn, $rs_bb_no, $rs_bb_code, "NO");
						...
						<td width="230" style="padding: 10px 10px 10px 10px">
							...
							* 적용 업체 리스트 <br>
							<select name="sel_right" size="10" style="width:230px" multiple>
								<?
									$arr_company_yes = listCompanyBoard($conn, $rs_bb_code, $rs_bb_no, "YES");
