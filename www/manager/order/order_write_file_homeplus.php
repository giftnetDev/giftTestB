	...
	#====================================================================
		...
		for($j = 0; $j <= 1; $j++){
			for ($i = 4; $i <= $data->sheets[$j]['numRows']; $i++) {
				...
				$cp_nm				  = trim($data->sheets[$j]['cells'][$i][1]);
				$cp_sub_nm			  = trim($data->sheets[$j]['cells'][$i][2]);
				$or_date		      = trim($data->sheets[$j]['cells'][$i][3]);
				$or_no		          = trim($data->sheets[$j]['cells'][$i][4]);
				$delivery_nm		  = trim($data->sheets[$j]['cells'][$i][5]);
				$delivery_no	      = trim($data->sheets[$j]['cells'][$i][6]);
				$delivery_code		  = trim($data->sheets[$j]['cells'][$i][7]);
				$delivery_status	  = trim($data->sheets[$j]['cells'][$i][8]);
				$or_nm			      = trim($data->sheets[$j]['cells'][$i][9]);
				$receiver_nm	      = trim($data->sheets[$j]['cells'][$i][10]);
				$zip			      = trim($data->sheets[$j]['cells'][$i][11]);
				$addr				  = trim($data->sheets[$j]['cells'][$i][12]);
				$phone				  = trim($data->sheets[$j]['cells'][$i][13]);
				$hphone	           	  = trim($data->sheets[$j]['cells'][$i][14]);
				$style_no	          = trim($data->sheets[$j]['cells'][$i][15]);
				$goods_code			  = trim($data->sheets[$j]['cells'][$i][16]);
				$cp_goods_code		  = trim($data->sheets[$j]['cells'][$i][17]);
				$size			      = trim($data->sheets[$j]['cells'][$i][18]);
				$goods_nm			  = trim($data->sheets[$j]['cells'][$i][19]);
				$option_nm		      = trim($data->sheets[$j]['cells'][$i][20]);
				$price		          = trim($data->sheets[$j]['cells'][$i][21]);
				$seller_price		  = trim($data->sheets[$j]['cells'][$i][22]);
				$or_cnt		          = trim($data->sheets[$j]['cells'][$i][23]);
				$total_price		  = trim($data->sheets[$j]['cells'][$i][24]);
				$delivery_request     = trim($data->sheets[$j]['cells'][$i][25]);
				$cp_code		      = trim($data->sheets[$j]['cells'][$i][26]);
				$virtual_account_date = trim($data->sheets[$j]['cells'][$i][27]);
				$item_seq             = trim($data->sheets[$j]['cells'][$i][28]);

				$addr			      = str_replace("?"," ",$addr);
				$cp_nm		          = str_replace("\"","",$cp_nm);
				$cp_sub_nm		      = str_replace("\"","",$cp_sub_nm);
				$delivery_nm		  = str_replace("\"","",$delivery_nm);
				$or_nm		          = str_replace("\"","",$or_nm);
				$receiver_nm		  = str_replace("\"","",$receiver_nm);
				$goods_nm		      = str_replace("\"","",$goods_nm);
				$option_nm		      = str_replace("\"","",$option_nm);

				$seller_price	= str_replace(",","",$seller_price);
				$seller_price	= str_replace("\"","",$seller_price);
				$seller_price	= trim($seller_price);			
				...
				$cp_sub_nm			  = SetStringToDB($cp_sub_nm);
				$or_date		      = SetStringToDB($or_date);
				$or_no		          = SetStringToDB($or_no);
				$delivery_nm		  = SetStringToDB($delivery_nm);
				$delivery_no	      = SetStringToDB($delivery_no);
				$delivery_code		  = SetStringToDB($delivery_code);
				$delivery_status	  = SetStringToDB($delivery_status);
				$or_nm			      = SetStringToDB($or_nm);
				...
				$style_no	          = SetStringToDB($style_no);
				$goods_code			  = SetStringToDB($goods_code);
				$cp_goods_code		  = SetStringToDB($cp_goods_code);
				$size			      = SetStringToDB($size);
				$goods_nm			  = SetStringToDB($goods_nm);
				$option_nm		      = SetStringToDB($option_nm);
				$price		          = SetStringToDB($price);
				$seller_price		  = SetStringToDB($seller_price);
				$or_cnt		          = SetStringToDB($or_cnt);
				$total_price		  = SetStringToDB($total_price);
				$delivery_request     = SetStringToDB($delivery_request);
				$cp_code		      = SetStringToDB($cp_code);
				$virtual_account_date = SetStringToDB($virtual_account_date);
				$item_seq             = SetStringToDB($item_seq);

				//전화번호와 휴대폰중 정보가 없는곳을 채워줌 - 2016-03-02 김지민요청
				if($phone != "" && $hphone == "")
					$hphone = $phone;
				...
				if($or_no <> '')
				{
					$temp_result = insertOrderHomeplus($conn, $file_nm, $cp_nm, $cp_sub_nm, $or_date, $or_no, $delivery_nm, $delivery_no, $delivery_code, $delivery_status, $or_nm, $receiver_nm, $zip, $addr, $phone, $hphone, $style_no, $goods_code, $cp_goods_code, $size, $goods_nm, $option_nm, $price, $seller_price, $or_cnt, $total_price, $delivery_request, $cp_code, $virtual_account_date, $item_seq, $s_adm_no);
				}
			}
		...
		$con_cp_no = '4';
		insertTempOrderHomeplus2Temp($conn, $con_cp_no, $file_nm, $s_adm_no);
...
<script language="javascript">
		location.href =  'order_write_file_homeplus.php?mode=L&temp_no=<?=$file_nm?>&this_date=<?=$this_date?>';
</script>
<?
		exit;
...
<script language="javascript">
	...
	function js_save() {
		...
		frm.method = "post";
		frm.action = "order_write_file_homeplus.php";
		frm.submit();
	}
	...
	*/
	...
	function js_reload() {
		location.href =  'order_write_file_homeplus.php?mode=L&temp_no=<?=$temp_no?>';
	}
	...
	table.rowstable04 td { color: #555555; text-align: center; vertical-align: middle; background: none; }
		...
		<td class="contentarea">
			...
			<div id="mwidthwrap">
				...
				<div class="sp20"></div>
				<div>
					* 전체 <?=totalCntTempOrderHomeplus($conn, $temp_no)?> 주문건수 중 &nbsp;&nbsp;
