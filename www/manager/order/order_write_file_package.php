...
# Confirm right
#==============================================================================
	$menu_right = "OD020"; // 메뉴마다 셋팅 해 주어야 합니다
...
#====================================================================
	...
	#====================================================================
		...
		for($i = 2 ; $i <= $data->sheets[0]['numRows']; $i++) {
			...
			if($cp_order_no != $prev_cp_order_no || $r_addr1 != $prev_r_addr1){
				$order_seq = 1;
				...
				insertTempOrderGoods($conn, $temp_no, $order_date, $prev_order_no, $order_seq, $goods_no, $goods_code, $goods_name, $goods_price, $qty,$opt_sticker_no, $opt_sticker_code, $opt_sticker_msg, $opt_outbox_tf, $opt_wrap_no, $opt_wrap_code, $opt_print_msg, $opt_outstock_date, $opt_memo, $delivery_type_code, $delivery_type, $delivery_price, $delivery_cp, $sender_nm, $sender_phone, $delivery_cnt_in_box);
			}
...
<style>
	...
	table.rowstable01 tr.third_tr td { background-color: #dfdfdf;}
	table.rowstable01 tr.end { border-bottom:2px solid black;}
				...
				<table cellpadding="0" cellspacing="0" class="rowstable01" style="width:1130px">
					...
					<thead>
						...
						<tr>
							...
							<th>출고예정일</th>
							<th>박스입수</th>
							<th class="end" colspan="5">작업메모</th>
						</tr>
					</thead>
					<tbody>
					<?
							...
							for ($j = 0 ; $j < sizeof($arr_rs); $j++) {
								...
								if (sizeof($arr_rs_temp_goods) > 0) {
									for ($k = 0 ; $k < sizeof($arr_rs_temp_goods); $k++) {
										...
										//} else if($GOODS_NO == "품절" || $GOODS_NO == "단종") {
										//	$err_str_order_goods .=  "상품판매상태 ".$GOODS_NO.",";
										} else
										{
								...
								if ($err_str_order <> "" && $err_str_order <> "주문정보 정상") {
					...
					?>
						...
						<tr height="30" class="third_tr end">
