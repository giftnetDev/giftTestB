...
<?
# =============================================================================
# File Name    : delivery_write_file_txt.php
# Modlue       : 
# Writer       : Park Chan Ho 
...
# Confirm right
#==============================================================================
	$menu_right = "OD008"; // 메뉴마다 셋팅 해 주어야 합니다
...
#====================================================================
	...
	#====================================================================
		...
		$file_dir = "../../upload_data/temp_delivery/".$file_nm;
...
*/
			if ($file_kind == "S") {
				...
				$order_goods_no		= trim($a_str[0]);
				$reserve_no				= trim($a_str[1]);
				$goods_name				= trim($a_str[2]);
				$qty							= trim($a_str[3]);
				$option						= trim($a_str[4]);
				$r_mem_nm					= trim($a_str[5]);
				$r_phone					= trim($a_str[6]);
				$r_zipcode				= trim($a_str[7]);
				$r_addr						= trim($a_str[8]);
				$delivery_cp			= trim($a_str[9]);
				$delivery_no			= trim($a_str[10]);
				$cp_no						= trim($a_str[11]);
				$buy_cp_no				= trim($a_str[12]);
				...
				$qty							= trim($a_str[3]);
				$option						= trim($a_str[4]);
				$r_mem_nm					= trim($a_str[8]);
				$r_phone					= trim($a_str[13]);
				$r_zipcode				= trim($a_str[9]);
				$r_addr						= trim($a_str[10]);
				$delivery_cp			= trim($a_str[16]);
				$delivery_no			= trim($a_str[17]);
				$cp_no						= trim($a_str[19]);
				$buy_cp_no				= trim($a_str[20]);
...
<script language="javascript">
	location.href =  'delivery_write_file_txt.php?mode=L&temp_no=<?=$file_nm?>';
</script>
<?
		exit;
...
<script language="javascript">
	...
	function js_save(f) {
		...
		frm.method = "post";
		frm.action = "delivery_write_file_txt.php";
		frm.submit();
	}
	...
	*/
	...
	function js_reload() {
		location.href =  'delivery_write_file_txt.php?mode=L&temp_no=<?=$temp_no?>';
	}
