...
# Confirm right
#==============================================================================
	$menu_right = "OD021"; // 메뉴마다 셋팅 해 주어야 합니다
...
?>
		...
		<td class="contentarea">
			...
			<div id="mwidthwrap">
				...
				<tbody>
					<tr>
						<th>판매업체</th>
						<td>
							<input type="text" class="seller" style="width:90%" placeholder="업체(명/코드) 입력후 엔터를 눌러주세요" name="txt_cp_type2" value="<?=getCompanyAutocompleteTextBox($conn,'',$cp_type2)?>" />
					...
					<? } ?>
						...
						<td>
							...
							<select name="search_field" style="width:100px;">
								...
								<option value="G.CP_ORDER_NO" <? if ($search_field == "CP_ORDER_NO") echo "selected"; ?> >업체주문번호</option>
								<option value="CP_ORDER_NO_MULTI" <? if ($search_field == "CP_ORDER_NO_MULTI") echo "selected"; ?> >업체주문번호(복수)</option>
								<option value="O_MEM_NM" <? if ($search_field == "O_MEM_NM") echo "selected"; ?> >주문자명</option>
								<option value="O.O_HPHONE" <? if ($search_field == "O.O_HPHONE") echo "selected"; ?> >주문자전화번호</option>
								<option value="R_MEM_NM" <? if ($search_field == "R_MEM_NM") echo "selected"; ?> >수령자명</option>
								<option value="O.R_HPHONE" <? if ($search_field == "O.R_HPHONE") echo "selected"; ?> >수령자전화번호</option>
								<option value="O.R_ADDR1" <? if ($search_field == "O.R_ADDR1") echo "selected"; ?> >수령자주소</option>
								<option value="GOODS_NAME" <? if ($search_field == "GOODS_NAME") echo "selected"; ?> >마트상품명</option>
								<option value="MART_GOODS_CODE" <? if ($search_field == "MART_GOODS_CODE") echo "selected"; ?> >마트상품번호</option>
								<option value="SUB_GOODS_NAME" <? if ($search_field == "SUB_GOODS_NAME") echo "selected"; ?> >구성상품명</option>
								<option value="SUB_GOODS_CODE" <? if ($search_field == "SUB_GOODS_CODE") echo "selected"; ?> >구성상품코드</option>
								<option value="GOODS_OPTION_NM_02" <? if ($search_field == "GOODS_OPTION_NM_02") echo "selected"; ?> >이마트배송번호</option>
							</select>&nbsp;
						...
						<td align="right">
							...
							<select name="print_type">
								<option value="screen">화면그대로</option>
								<option value="selected">선택된상품만</option>
							</select>
							<a href="javascript:js_excel();"><img src="../images/common/btn/btn_excel.gif" alt="엑셀 리스트" /></a><br/>
							-->
						</td>
					</tr>
			...
			<b>총 주문 <?=$nListCnt?> 건, 총 주문상품 <?=$nListCnt2?> 건</b>
			&nbsp;&nbsp;&nbsp;&nbsp;
			...
			<!--<b><font color="blue">배송완료후</font> <font color="red"><?=$cnt_3?></font> <font color="blue">건</font></b>&nbsp;&nbsp;-->
						...
						<tr class="<?=$str_tr?> <?=$str_cancel_style?> <?=$soldout_goods?> <?=$style_goods_state?>"  height="35">
							...
							<td class="modeual_nm">
								...
								<? } else { ?>
									주문확인
								<? } ?>
								<?=$GOODS_STATE != '판매중' && $GOODS_STATE != '재판매' ? "(".$GOODS_STATE.")" : ""?>
							</td>
							<? if ($s_adm_cp_type == "운영") { ?>
										...
										if(sizeof($arr_delivery) > 0) {
											for($k = 0; $k < sizeof($arr_delivery); $k++) {
												...
												if($rs_use_tf == 'N') { 
													...
													$str_delivery_state = "<span style='color:gray;'>취소</span>";
												} else {
													if($rs_delivery_date != "0000-00-00") {
														$style_delivery_state = "style='color:green;'";
														$str_delivery_state = "<span style='color:green;'>발송</span>";
													} else { 	
														$style_delivery_state = "style='color:red;'";
														$str_delivery_state = "<span style='color:red;'>발송전</span>";
													}
												}
