...
#============================================================
	...
	$homepage_filter = "";	//홈페이지 관련 사항 없음. 20210715
...
#===============================================================
	...
	if($sel_order_state.$cp_type.$cp_type2.$sel_cate_01.$sel_sale_confirm_tf.$con_work_flag.$sel_opt_manager_no.$sel_delivery_type. $sel_delivery_cp.$search_str <> "" ) { 
		if($start_date == $day_31 && $end_date == $day_0 && $cp_type2 == "") {
			$start_date = $day_180;
			$end_date = $day_0;
		} 
	}
...
<script language="javascript">
	...
	*/
	...
	function js_open_manual(){
		NewWindow("delivery_list_manual.php","delivery_list_manual","800","500");
	}
...
?>
		...
		<td class="contentarea">
			...
			<div id="mwidthwrap">
				...
				<h2>배송 리스트 &nbsp; <input type="button" value="참고사항" onclick="js_open_manual()"></h2>
				<div class="btnright">
				...
				<?
					...
					if (sizeof($arr_rs) > 0) {
						...
						<tr class="order <?=$str_cancel_style?>" height="35">
							<td class="order"><a href="javascript:js_view('<?=$rn?>','<?=$RESERVE_NO?>');"><?=$RESERVE_NO?></a></td><!--주문번호-->
							<td class="modeual_nm"><?= getCompanyName($conn, $CP_NO);?></td><!--판매업체명-->
							<td><?=$O_MEM_NM?></td><!--주문자명-->
							<td><?=$O_PHONE?></td><!--주문자 연락처-->
							<td><?=$R_MEM_NM?></td><!--수령자명-->
							<td><?=$R_ZIPCODE?></td><!--우편번호-->
							<td colspan="4" class="modeual_nm"><?=$R_ADDR1?></td><!--주소-->
							...
							<td><?=$R_PHONE?></td><!--수령자연락처-->
							<td colspan="2" class="filedown"><?=$ORDER_DATE?></td><!--주문일시-->
						</tr>
						<?
							...
							if (sizeof($arr_goods) > 0) {
								for ($h = 0 ; $h < sizeof($arr_goods); $h++) {
									...
									if($OPT_STICKER_NO <> "0" || $OPT_OUTBOX_TF == "Y" || $OPT_WRAP_NO <> "0" || $OPT_PRINT_MSG <> "" || $OPT_MEMO<>"" || $OPT_SUPPORT_MEMO <>"" || $OPT_REQUEST_MEMO<>"") {
						...
						<tr class="<?=$str_tr?> <?=$str_cancel_style?>" height="35" title="<?=($ORDER_STATE == "3" ? "배송완료일시:".$FINISH_DATE : "")?>">
							...
							<td class="modeual_nm"><?= getCompanyName($conn, $BUY_CP_NO);?></td><!--공급업체-->
							<td class="modeual_nm" colspan="2"><?=$option_str?></td><!--옵션/출고예정일-->
