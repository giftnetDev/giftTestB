		...
		<?
			...
			<tr>
				...
				<td class="line">
					<!--주문확인 전에만 수량 변경 가능-->
					<? if($ORDER_STATE == "1") {  ?>
						<input type="text" id="qty" name="qty" value="<?=$QTY?>" style="width:90px" onkeyup="return isNumber(this)" onChange="js_calculate_all_price()"/> 개
						<font class="price" color="gray" style="font-size:0.9em;">(<?=$QTY?> 개)</font>
					<? } else {?>
				...
				<td class="line">
					...
					<input type="hidden" name="old_discount_price" value="<?=$DISCOUNT_PRICE?>">
					<input type="hidden" name="old_delivery_cnt_in_box" value="<?=$DELIVERY_CNT_IN_BOX?>">
					<input type="hidden" name="order_state" value="<?=$ORDER_STATE?>">
					<input type="hidden" name="delivery_type" value="<?=$DELIVERY_TYPE?>">

					<input type="text" class="txt" style="width:90px" name="sale_price" value="<?=$SALE_PRICE?>" onkeyup="return isPhoneNumber(this)" onChange="js_calculate_all_price()"> 원
				</td>
			</tr>
			...
			<tr>
				<th>할인금액</th>
				<td class="line">
					<input type="text" id="discount" class="txt" style="width:90px" name="discount_price" value="<?=$DISCOUNT_PRICE?>" onkeyup="return isNumber(this)" onChange="js_calculate_all_price()" 
			...
			<!-- tr>
				...
				<td class="line">
					...
					<?=number_format($DELIVERY_CNT_IN_BOX)?> 
			...
			<div class="sp10"></div>
			<div class="btn">
			<!--2017-08-17 취소나 교환에 대한 가격변경 금지 .. 클레임시 원장 ORDER_GOODS_NO에 정확한 정보가 없음 + INPUT_TYPE -->
			<? if (!(($ORDER_STATE == "4") || ($ORDER_STATE == "6") || ($ORDER_STATE == "7") || ($ORDER_STATE == "8"))) { ?>
			<? if ($sPageRight_U == "Y") {?>
