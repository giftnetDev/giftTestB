...
?>
<?// function ZONE
	function cntHompageOrder($db){
		$query="SELECT 	COUNT(DISTINCT G.RESERVE_NO) AS CNT
				FROM 	TBL_ORDER O
				JOIN	TBL_COMPANY C ON O.CP_NO = C.CP_NO
				JOIN 	TBL_ORDER_GOODS	G ON O.RESERVE_NO = G.RESERVE_NO
				WHERE	O.HOMEPAGE_YN='Y'
				AND		G.ORDER_STATE='1'
				AND		G.USE_TF='Y'
				AND		O.USE_TF='Y'
				AND		O.DEL_TF='N'
...
<?
	...
	if ($mode == "CU") { //주문확인
		...
		for ($k = 0; $k < $row_cnt; $k++) {
			...
			$temp_order_no			= trim($arr_order_goods_no[0]);//RESERVE_NO
			$temp_order_goods_no	= trim($arr_order_goods_no[1]);//ORDER_GOODS_NO
			$temp_cate_01			= trim($arr_order_goods_no[2]);//CATE_O1 ={'','증정','추가','샘플'}
			$temp_reg_adm			= trim($arr_order_goods_no[3]);//s_adm_no정보
			...
			$temp_goods_name		= ($cate_01 <> "" ? $cate_01.")" : "").SetStringFromDB($arr_order_goods[0]["GOODS_NAME"])."[".SetStringFromDB($arr_order_goods[0]["GOODS_CODE"])."]";
			...
			if($temp_sale_confirm_tf == "N") { 
				...
				//if($base_date < $chk_reg_date) {
					...
					}//end of if($refund_able_qty>0 && $temp_sale_price>0);
				//}
			}//end of if(sale_confirm_tf=='N')

		}//end of for(주문확인으로 바꾸려는 )
	}//end of mode "CU"
	...
	if($mode == "SALE_CONFIRM") { 
		...
		if(sizeof($arr_memo) > 0) { 
			$A = SetStringFromDB($arr_memo[0]["A"]);
			//$B = $arr_memo[0]["B"];
			$C = SetStringFromDB($arr_memo[0]["C"]);
			$D = SetStringFromDB($arr_memo[0]["D"]);
...
#===============================================================
	...
	// 	} 
	// }
	$cntHomepageOrder=cntHompageOrder($conn);

	$nListCnt = totalCntManagerDelivery($conn, $search_date_type, $start_date, $end_date, $bulk_tf, $sel_order_state, $cp_type, $cp_type2, $sel_cate_01, $sel_sale_confirm_tf, $con_work_flag, $sel_opt_manager_no, $sel_delivery_type, $sel_delivery_cp, $con_use_tf, $del_tf, $search_field, $search_str, $homepage_filter);
	...
	// echo "nPage : $nPage, nPageSize : $nPageSize<br>";

	$arr_rs = listManagerDelivery2($conn, $search_date_type, $start_date, $end_date, $bulk_tf, $sel_order_state, $cp_type, $cp_type2, $sel_cate_01, $sel_sale_confirm_tf, $con_work_flag, $sel_opt_manager_no, $sel_delivery_type, $sel_delivery_cp, $con_use_tf, $del_tf, $search_field, $search_str, $order_field, $order_str, $nPage, $nPageSize, $homepage_filter);
...
<script language="javascript">
	...
	function js_view_only_hompage_order(){
		var frm=document.frm;
		frm.homepage_filter.value="FILTERING";
		frm.nPage.value="1";
		frm.method="POST";
		frm.target="";
		frm.action="<?=$_SERVER['PHP_SELF']?>";
		...
		<td class="contentarea">
			...
			<div>
				<style>
					...
					span#cntHomepageOrder:hover{
						background-color: #EEEEEE;
					}
				</style>
				<?if($cntHomepageOrder>0){?><span id="cntHomepageOrder" onclick="js_view_only_hompage_order()" style="">홈페이지 총 <?=$cntHomepageOrder?> 건</span><?}?>
				<input type="hidden" name="homepage_filter">				
			</div>
			<div class="sp10"></div>
				...
				<?
					...
					if (sizeof($arr_rs) > 0) {
						for ($j = 0 ; $j < sizeof($arr_rs); $j++) {
							...
							$HOMEPAGE_YN						= trim($arr_rs[$j]["HOMEPAGE_YN"]);
							$CALL_YN							= trim($arr_rs[$j]["CALL_YN"]);
						...
						<tr class="order <?=$str_cancel_style?>" height="35">
							...
							?>
								<td class="order"><a href="javascript:js_view('<?=$rn?>','<?=$RESERVE_NO?>');"><font color="blue"><?=$RESERVE_NO?></font></a>
								☎</td>	
							<?	}
								else
								{
							?>	
								<td class="order"><a href="javascript:js_view('<?=$rn?>','<?=$RESERVE_NO?>');"><font color="blue"><?=$RESERVE_NO?></font></a></td>		
							<?		
								}
							}
							else{
						...
						<?
							...
							if (sizeof($arr_goods) > 0) {
								...
								for ($h = 0 ; $h < sizeof($arr_goods); $h++) {
									// echo "TEST2";
									// echo "size_of_arr_goods : ".sizeof($arr_goods)."<br>";
									$OG_RESERVE_NO				= trim($arr_goods[$h]["RESERVE_NO"]);
									if($RESERVE_NO != $OG_RESERVE_NO){
										// echo "OG_RESERVE_NO : ".$OG_RESERVE_NO." || RESERVE_NO : $RESERVE_NO <br>";
										// echo "continue<br>";
										continue;
									}
						...
						<tr class="<?=$str_tr?> <?=$str_cancel_style?>" id="colorTr" height="35">
							<td class="modeual_nm">
