<?
	...
	function selectCustomer($db, $gr_no, $search_field, $search_str, $nPage, $nRowCount, $total_cnt) {
		...
		$query = "SELECT  @ROWNUM := @ROWNUM +1 AS RN
						...
						, A.USE_TF
						, A.DEL_TF
				    FROM TBL_CUSTOMER  A, (SELECT @rownum:=0) B 
				   WHERE A.GROUP_NO = '0' 
					 AND A.USE_TF =  'Y'
					 AND A.DEL_TF =  'N'
				";
		...
		$query .= " ORDER BY CUSTOMER_NO limit ".$offset.", ".$nRowCount;
	...
	function totalCntCustomer($db, $gr_no, $search_field, $search_str){
	...
	function updateCompany($db, $cp_cate, $cp_type, $cp_nm, $cp_nm2, $cp_code, $cp_phone, $cp_hphone, $cp_fax, $cp_zip, $cp_addr, $re_zip, $re_addr, $homepage, $biz_no, $ceo_nm, $upjong, $uptea, $dc_rate, $sale_adm_no, $manager_nm, $phone, $hphone, $fphone, $email, $email_tf, $contract_start, $contract_end, $ad_type, $account_bank, $account, $account_owner_nm, $is_mall, $memo, $use_tf, $up_adm, $cp_no) {
