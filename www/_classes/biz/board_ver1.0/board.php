<?
  ...
  PRIMARY KEY (`FILE_NO`)
	...
	*/
	...
	function insertBoard($db, $bb_code, $cate_01, $cate_02, $cate_03, $cate_04, $writer_nm, $writer_pw, $email, $homepage, $title, $ref_ip, $recomm, $contents, $file_nm, $file_rnm, $file_path, $file_size, $file_ext, $keyword, $comment_tf, $top_tf, $use_tf, $reg_adm) {
	...
	function insertBoardReply($db, $bb_code, $bb_no, $bb_po, $bb_re, $bb_de, $cate_01, $cate_02, $cate_03, $cate_04, $writer_nm, $writer_pw, $email, $homepage, $title, $ref_ip, $recomm, $contents, $file_nm, $file_rnm, $file_path, $file_size, $file_ext, $keyword, $comment_tf, $top_tf, $use_tf, $reg_adm) {
	...
	function insertBoardFile($db, $bb_code, $bb_no, $file_nm, $file_rnm, $file_path, $file_size, $file_ext, $reg_adm) {
				
		$query = "INSERT INTO TBL_BOARD_FILE (BB_CODE, BB_NO, FILE_NM, FILE_RNM, FILE_PATH, FILE_SIZE, FILE_EXT, HIT_CNT, REG_ADM, REG_DATE) 
														values ('$bb_code','$bb_no', '$file_nm', '$file_rnm', '$file_path', '$file_size', '$file_ext', '0', '$reg_adm', now()); ";
		//echo $query;
		//exit;
	...
	function getScrapCount($db, $bb_code, $bb_no, $WRITE_ID) {

		$query = "SELECT COUNT(BB_NO) AS CNT FROM TBL_Scrap WHERE WRITER_ID='$WRITE_ID' AND BB_CODE = '$bb_code' AND S_BB_NO = '$bb_no' AND DEL_TF = 'N' ";
	...
	function insertScrap($db, $bb_code, $bb_no, $writer_id, $title, $content, $del_tf) {
		
		$query ="SELECT IFNULL(MAX(BB_NO),0) AS MAX_NO FROM TBL_Scrap";
		$result = mysql_query($query,$db);
		$rows   = mysql_fetch_array($result);
		...
		$query5="INSERT INTO TBL_Scrap (BB_NO, BB_CODE, S_BB_NO, CATE_01, WRITER_ID, CONTENTS, DEL_TF, REG_DATE) 
														values ('$new_bb_no', '$bb_code', '$bb_no', '$title', '$writer_id', '$content','$del_tf', now()); ";
	...
	function totalCntScrap($db, $s_mem_id, $del_tf){

		$query ="SELECT COUNT(*) CNT FROM TBL_Scrap WHERE WRITER_ID = '$s_mem_id' ";
	...
	function listScrap($db, $s_mem_id, $del_tf, $nPage, $nRowCount) {

		$total_cnt = totalCntScrap($db, $s_mem_id,  $del_tf);
		...
		$query = "SELECT @rownum:= @rownum - 1  as rn, BB_NO, BB_CODE, S_BB_NO, CATE_01, CATE_02, CATE_03, CATE_04, 
										 WRITER_ID, CONTENTS, USE_TF, DEL_TF, REG_ADM, REG_DATE, UP_ADM, UP_DATE, DEL_ADM, DEL_DATE
										 FROM TBL_Scrap WHERE WRITER_ID = '$s_mem_id' ";
	...
	function BB_CODE_NAME($db, $bb_code){

		$query ="SELECT BOARD_NM FROM TBL_BOARD_CONFIG WHERE BOARD_CODE = '$bb_code' ";
	...
	function deleteScrap($db, $bb_no) {

		$query = "UPDATE TBL_Scrap SET 
															DEL_TF = 'Y'
									WHERE BB_NO					= '$bb_no' ";
	...
	function listBoardClaim($db, $start_date, $end_date, $bb_code, $cate_01, $cate_02, $cate_03, $cate_04, $keyword, $reply_state, $center_cd, $order_path, $use_tf, $del_tf, $search_field, $search_str, $nPage, $nRowCount) {

		$total_cnt = totalCntBoardClaim($db, $start_date, $end_date, $bb_code, $cate_01, $cate_02, $cate_03, $cate_04, $keyword, $reply_state, $center_cd, $order_path, $use_tf, $del_tf, $search_field, $search_str);
		...
		$query = "SELECT @rownum:= @rownum - 1  as rn, A.BB_CODE, A.BB_NO, A.BB_PO, A.BB_RE, A.BB_DE, A.CATE_01, A.CATE_02, A.CATE_03, A.CATE_04, 
										 A.WRITER_NM, A.WRITER_PW, A.EMAIL, A.HOMEPAGE, A.TITLE, A.HIT_CNT, A.REF_IP, A.RECOMM, A.CONTENTS,
										 A.FILE_NM, A.FILE_RNM, A.FILE_PATH, A.FILE_SIZE, A.FILE_EXT, A.KEYWORD, A.REPLY, A.REPLY_ADM, A.REPLY_DATE, A.REPLY_STATE, A.COMMENT_TF, TOP_TF,
										 A.USE_TF, A.DEL_TF, A.REG_ADM, A.REG_DATE, A.UP_ADM, A.UP_DATE, A.DEL_ADM, A.DEL_DATE,
										 O.ORDER_DATE 
								FROM TBL_BOARD A, TBL_ORDER O WHERE A.CATE_01 = O.RESERVE_NO ";
		...
		if ($keyword <> "") {
			$query .= " AND A.KEYWORD  = '".$keyword."' ";
			//$query .= " AND ( (A.KEYWORD like '%".$keyword."%') or (A.TITLE like '%".$keyword."%') or (A.WRITER_NM like '%".$keyword."%')) ";
		}

		if ($center_cd <> "") {
			$query .= " AND O.CENTER_CD = '".$center_cd."' ";
		} 
		...
		if ($order_path <> "") {
			if ($order_path == "web") {
				$query .= " AND O.REG_ADM = '".$order_path."' ";
			} else {
				$query .= " AND O.REG_ADM <> 'web' ";
			}
		}
	...
	function totalCntBoardClaim($db, $start_date, $end_date, $bb_code, $cate_01, $cate_02, $cate_03, $cate_04, $keyword, $reply_state, $center_cd, $order_path, $use_tf, $del_tf, $search_field, $search_str){
