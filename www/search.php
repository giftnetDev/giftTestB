...
<script>
	function js_order_by(order_field, order_str) {
		...
		location.href ="<?=$_SERVER[PHP_SELF]?>?cate=<?=$cate?>&order_field=" + order_field + "&order_str=" + order_str + "&search_str=<?=$search_str?>";

	}

	function js_page_size(pagesize) { 
		location.href ="<?=$_SERVER[PHP_SELF]?>?cate=<?=$cate?>&order_field=<?=$order_field?>&order_str=<?=$order_str?>&nPageSize=" + pagesize + "&search_str=<?=$search_str?>";
	}
    ...
    <nav class="text-center">
        ...
        <?
			...
			if (sizeof($arr_rs) > 0) {
				...
				$strParam = "";
				$strParam = $strParam."&cate=".$cate."&nPageSize=".$nPageSize."&search_field=".$search_field."&search_str=".$search_str."&order_field=".$order_field."&order_str=".$order_str;
