...
#===============================================================
	...
	if($is_catalog) { 
		...
		$nListCnt =totalCntGoods($conn, $con_cate, $start_date, $end_date, $start_price, $end_price, $con_cate_01, $con_cate_02, $con_cate_03, '판매중', $con_tax_tf, $con_use_tf, $del_tf, $search_field, $search_str, $arr_options, $order_field, $order_str);
		...
		$arr_rs = listGoods($conn, $con_cate, $start_date, $end_date, $start_price, $end_price, $con_cate_01, $con_cate_02, $con_cate_03, '판매중', $con_tax_tf, $con_use_tf, $del_tf, $search_field, $search_str, $arr_options, $order_field, $order_str, $nPage, $nPageSize);
...
<script>
	function js_order_by(order_field, order_str) {
		...
		location.href ="<?=$_SERVER[PHP_SELF]?>?cate=<?=$cate?>&order_field=" + order_field + "&order_str=" + order_str;

	}

	function js_page_size(pagesize) { 
		location.href ="<?=$_SERVER[PHP_SELF]?>?cate=<?=$cate?>&order_field=<?=$order_field?>&order_str=<?=$order_str?>&nPageSize=" + pagesize;
	}

	function js_cate_page(cate_page) { 
		location.href ="<?=$_SERVER[PHP_SELF]?>?cate=<?=$cate?>&order_field=<?=$order_field?>&order_str=<?=$order_str?>&nPageSize=<?=$nPageSize?>&cate_page=" + cate_page;
	}
</script>
</head>
<body>
...
<div class="container-fluid" id="prod-list">
    <div class="container">
        <div class="row prod-top">
            ...
            <div class="text-right col-lg-7 prod-view-option">
				<? if($is_catalog) { ?>
                ...
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                        <li role="presentation"><a href="javascript:js_page_size('15');">15개씩 보기</a></li>
                        <li role="presentation"><a href="javascript:js_page_size('30');"">30개씩 보기</a></li>
                        <li role="presentation"><a href="javascript:js_page_size('90');"">90개씩 보기</a></li>
                    </ul>
                </div>
            </div>
        </div>
		...
		<?
			...
			for ($j = 0 ; $j < sizeof($arr_rs); $j++) {
				...
				// 이미지가 저장 되어 있을 경우
				$img_url	= getGoodsImage($FILE_NM, $IMG_URL, $FILE_PATH_150, $FILE_RNM_150, "188", "188");
		...
		?>
            <div class="col-lg-2 item">
                <a href="detail1.php?cate=<?=$cate?>&goods_no=<?=$GOODS_NO?>" class="thumbnail">
					<img src="<?=$img_url?>" alt="<?=$GOODS_NAME?>" style="min-width:188px; min-height:188px; max-width:188px; max-height:188px;">
    ...
    <nav class="text-center">
        ...
        <?
			...
			if (sizeof($arr_rs) > 0) {
				...
				$strParam = $strParam."&cate=".$cate."&cate_page=".$cate_page;
