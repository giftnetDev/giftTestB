	...
	function getOrderGoodsInfo($db, $orderGoodsNo){
		...
		$query	="	SELECT 	OG.GOODS_NO, OG.GOODS_CODE, OG.GOODS_NAME, OG.GOODS_SUB_NAME, OG.QTY, OG.ORDER_STATE, OG.SALE_PRICE, OG.DELIVERY_PRICE, OG.DISCOUNT_PRICE,
							...
							, (SELECT GOODS_NAME
											FROM TBL_GOODS
											WHERE GOODS_CATE LIKE'0103%'
											AND GOODS_NO=OG.OPT_STICKER_NO
											AND USE_TF='Y'
											AND DEL_TF='N') AS OPT_STICKER_NAME
...
//GOODS_NO, GOODS_CODE, GOODS_NAME, SALE_PRICE, DELIVERY_CNT_IN_BOX
		...
		$msg=$_POST['optPrintMsg'];

		// $cntMsg=strlen($msg);
		// for($i=0; $i<$cntMsg; $i++){
		// 	$list->insertAtBack(ord(substr($msg, $i, 1)));

		// }
		// $list->printNode();
		// exit;
		$msg=str_replace("\n","<br>",$msg);
		$optPrintMsg=trim(SetStringToDB(iconv("utf8","euckr",$msg)));
		// $optPrintMsg = str_replace("\r\n","<br>",$_POST['optPrintMsg']);
		// $optPrintMsg = iconv("utf8","euckr",str_replace("\r\n","<br>",$_POST['optPrintMsg']));


		//  echo $optPrintMsg; 
		//  exit;
		// $optPrintMsg=	trim(SetStringToDB(iconv("utf8","euckr",$optPrintMsg)));
		// $optPrintMsg=	str_replace("\r\n","<br>",$optPrintMsg);
		...
		$goods["OPT_PRINT_MSG"]=str_replace("\r\n",'<br>',$msg);

		// $goods["OPT_PRINT_MSG"]   =str_replace("\r\n",'<br>', $goods["OPT_PRINT_MSG"]);

		// $goods["OPT_PRINT_MSG"]=str_replace("\r\n","<br>",$goods["OPT_PRINT_MSG"]);
		// $goods["OPT_PRINT_MSG"]=str_replace("\n","<br>",$goods["OPT_PRINT_MSG"]);
		// echo $goods["OPT_PRINT_MSG"];
		// exit;
		...
		$goodsInfo["OPT_STICKER_NM"]=	urlencode(iconv("euckr","utf8",$goods["OPT_STICKER_NAME"]));
		$goodsInfo["REG_DATE"]	=		urlencode(iconv("euckr","utf8",$goods["REG_DATE"]));
		...
		if($goodsInfo["OPT_STICKER_NO"] > 0 || $goodsInfo["OPT_PRINT_MSG"])
		{
			...
			$optStickerNo=$goodsInfo["OPT_STICKER_NO"];

			$optPrintMsg=SetStringFromDB($goodsInfo["OPT_PRINT_MSG"]);

			if($optStickerNo>0)
			{
				$optSticker=SetStringFromDB(getStickerNameByNo($conn, $optStickerNo));
				
					$optstr.="스티커 옵션 : ".$optSticker."<br>";				
			}

			if($optPrintMsg <> "" )
			{
			
				$optstr.="인쇄 옵션 : ".$goods["OPT_PRINT_MSG"]."<br>";
			}
		}
		else
		{
		...
		$goodsInfo["OPTION_COMMENT"]	=		urlencode(iconv("euckr","utf8",$opt));
