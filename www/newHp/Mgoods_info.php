<?require "../_common/home_pre_setting.php";
	...
	function setlGoodsMenu($db, $goods_no) 
	{
		$query = " 	SELECT GROUP_CONCAT( CATE_CODE SEPARATOR ' > ' ) AS CATE_CODE
						 , GROUP_CONCAT( SUBSTRING( CATE_NAME, INSTR( CATE_NAME, '. ' ) +1, LENGTH( CATE_NAME ) ) SEPARATOR ' >' ) AS CATE_NAME
					  FROM TBL_CATEGORY
					 WHERE 1 =1
					   AND DEL_TF = 'N'
					   AND USE_TF = 'Y'
					   AND CATE_CD LIKE '15%'
					   AND CATE_CODE IN (
											SELECT SUBSTRING( GOODS_CODE, 1, 3 ) AS SUB_MENU_CD
											  FROM TBL_GOODS
											 WHERE GOODS_NO = '$goods_no'
											 UNION
											SELECT CONCAT( SUBSTRING( GOODS_CODE, 1, 1 ) , '00' ) AS MAIN_MENU_CD
											  FROM TBL_GOODS
											 WHERE GOODS_NO = '$goods_no'
										)
				";
...
<?
	...
	$menu_nm=setlGoodsMenu($conn, $goods_no);
...
function js_add_goods_to_wishListM(goodsNo, imgUrl, memberNo, cpNo, obj)
{
	//alert(memberNo);
    if(memberNo==""){
...
<div class="wrap main_sub_wrap">
	<div class="main_sub_location">
		<?=$menu_nm?>
	</div>
	<div class="sub_wrap">
		...
		<form name="frm">
			...
			<div class="sub_img" style="background:url('<?=$img_url?>') no-repeat;background-size:cover;background-position:center center;"></div>
				<span><?=$rs_goods_code?></span>
				<?
					if($tmpCntLike>0 && $memberNo>0){
						if(isExistingAtWishList($conn, $goods_no, $memberNo)>0){
						?>
							<div class="like like_on" id="likeSymbol_<?=$goods_no?>" onclick="js_add_goods_to_wishListM('<?=$goods_no?>', '<?=$img_url?>', '<?=$_SESSION['C_MEM_NO']?>', '<?=$_SESSION['C_CP_NO']?>', this)"></div>
						<?
							$tmpCntLike--;
						}
						else{
						?>
							<div class="like" id="likeSymbol_<?=$goods_no?>" onclick="js_add_goods_to_wishListM('<?=$goods_no?>', '<?=$img_url?>', '<?=$_SESSION['C_MEM_NO']?>', '<?=$_SESSION['C_CP_NO']?>', this)"></div>
						<?
						}
					}
					else{
						...
						<table>
							...
							<tr>
								...
								?>
									<td>
										<span>
											<strong id="sale_price" data-sale_price="<?=$rs_sale_price?>"><?=number_format($rs_sale_price)?></strong>
										</span> 원
									</td>
								<?
								}
							...
							<tr>
								<th>주문수량</th>
								<!--<td><div class="count_up_down"><div class="count_up_down_down">&lt;</div><span>1</span><div class="count_up_down_up">&gt;</div></div></td>-->
								<td>
				...
				<div class="detail_itself">
					...
					?>
						...
						<div class="product_rest">
							<a href="Mgoods_info.php?goods_no=<?=$GOODS_NO?>" class="thumbnail">
